...
import apiDepartment from '@/api/modules/departmentExample.ts'
...
function getTitle() {
  apiDepartment.detail(route.params.department_id as string).then((res) => {
...
<template>
  <div :class="{ 'absolute-container': tableAutoHeight }">
    <FaPageHeader :title="`「${title}」职位管理`" class="mb-0">
      <FaButton
        variant="outline" size="sm" class="rounded-full" @click="router.close({
